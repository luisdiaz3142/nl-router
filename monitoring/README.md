...
Both run under host networking so they reach `localhost:9180-9184` as
if they were the host process. Persistent state lives in the named
Docker volumes `prometheus_data` and `grafana_data`.
...
- The nl-router daemons running on this host with their default
  metrics ports (9180-9184). Override `NL_ROUTER_METRICS_PORT` in
  each systemd unit if your site uses different ports — then update
...
### Adding module-worker scrapes

Module workers (`nl-mod-anonymize-basic`, etc.) default
`NL_ROUTER_METRICS_PORT=0` (exposer disabled). To monitor them:

1. Pick a port per kind (the convention is `9190+`):
   ```sh
   sudo systemctl edit nl-router-module@anonymize-basic.service
   ```
   Add:
   ```ini
   [Service]
   Environment="NL_ROUTER_METRICS_PORT=9190"
   ```
2. Restart the worker.
3. Uncomment the matching entry in `prometheus.yml` and reload
   Prometheus.
